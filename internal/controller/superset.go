	...
	return r.apply(ctx, dp, sts, func() error {
		...
		sts.Spec.Template.Spec.Containers = []corev1.Container{{
			...
			Env: []corev1.EnvVar{
				...
				{Name: "POSTGRES_DB", Value: supersetPostgresDatabase(spec)},
...
func (r *DataPlatformReconciler) supersetConfig(
	...
	if oidc.publicIssuer != "" && oidc.publicIssuer != oidc.issuer {
		// Browser token exchange can use the public issuer; in-cluster refresh
		// still works against Keycloak via the same hostname when backchannel is on.
		tokenURL = oidcTokenURL(browserIssuer)
	}
	jwksURL := oidcJWKSURL(oidc.issuer)
	serverMetadata := strings.TrimRight(browserIssuer, "/") + "/.well-known/openid-configuration"
	apiBase := strings.TrimRight(browserIssuer, "/") + "/protocol/openid-connect/"
	...
	if oidc.enabled {
		...
		b.WriteString("        \"client_kwargs\": {\"scope\": \"openid profile email offline_access\"},\n")
		b.WriteString(fmt.Sprintf("        \"api_base_url\": %q,\n", apiBase))
		b.WriteString(fmt.Sprintf("        \"server_metadata_url\": %q,\n", serverMetadata))
		...
		b.WriteString("        \"scope\": \"openid profile email offline_access\",\n")
		if publicURL != "" {
...
func supersetBootstrapTrino(dp *dataplatformv1alpha1.DataPlatform) string {
	...
	b.WriteString("    try:\n")
	b.WriteString("        from flask import current_app\n")
	b.WriteString("        if current_app.config.get(\"DATABASE_OAUTH2_CLIENTS\"):\n")
	b.WriteString("            # Superset 5 stores encrypted OAuth2 client linkage on the DB row via extras.\n")
	b.WriteString("            encrypted = database.encrypted_extra or {}\n")
	b.WriteString("            if isinstance(encrypted, str):\n")
	b.WriteString("                encrypted = __import__(\"json\").loads(encrypted or \"{}\")\n")
	b.WriteString("            encrypted[\"oauth2_client_info\"] = {\"id\": \"Trino\"}\n")
	b.WriteString("            database.encrypted_extra = __import__(\"json\").dumps(encrypted)\n")
